...
local Webhook = 'Please Put CZDEV'
local staffs = {}
local FeedbackTable = {}

-------------------------- NEW FEEDBACK

RegisterNetEvent("czReports:NewFeedback")
AddEventHandler("czReports:NewFeedback", function(data)
	...
	TriggerClientEvent("czReports:NewFeedback", -1, newFeedback)
...
RegisterNetEvent("czReports:FetchFeedbackTable")
AddEventHandler("czReports:FetchFeedbackTable", function()
	local staff = hasPermission(source)
	if staff then
		staffs[source] = true
		TriggerClientEvent("czReports:FetchFeedbackTable", source, FeedbackTable, staff, oneSync)
	end
end)

-------------------------- ASSIST FEEDBACK

RegisterNetEvent("czReports:AssistFeedback")
AddEventHandler("czReports:AssistFeedback", function(feedbackId, canAssist)
	if staffs[source] then
		if canAssist then
			...
			if GetPlayerPing(id) > 0 then
				...
				TriggerClientEvent('okokNotify:Alert', source, "czReports", "You are assisting FEEDBACK #"..feedbackId.."!", 20000, 'info')
				TriggerClientEvent('okokNotify:Alert', id, "czReports", "An admin arrived!", 20000, 'info')

				if Webhook ~= '' then
					assistFeedbackWebhook(assistFeedback)
				end
			else	
				TriggerClientEvent('okokNotify:Alert', id, "czReports", "That player is no longer in the server!", 20000, 'error')
			end
			if not FeedbackTable[feedbackId].concluded then
				FeedbackTable[feedbackId].concluded = "assisting"
			end
			TriggerClientEvent("czReports:FeedbackConclude", -1, feedbackId, FeedbackTable[feedbackId].concluded)
		end
	end
end)
...
RegisterNetEvent("czReports:FeedbackConclude")
AddEventHandler("czReports:FeedbackConclude", function(feedbackId, canConclude)
	if staffs[source] then
