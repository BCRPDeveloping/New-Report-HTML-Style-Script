Citizen.CreateThread(function()
	Wait(1000)
	TriggerServerEvent("czReports:FetchFeedbackTable")
end)
...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		exports['okokNotify']:Alert("czReports", "You can't report so quickly!", 10000, 'warning')
	end
end, false)
...
AddEventHandler('czReports:NewFeedback', function(newFeedback)
	if staff then
		FeedbackTable[#FeedbackTable+1] = newFeedback
exports['okokNotify']:Alert("czReports", "New Report Some one needs help!", 20000, 'success')
		SendNUIMessage({
			action = "updateFeedback",
			FeedbackTable = FeedbackTable
		})
...
AddEventHandler('czReports:FetchFeedbackTable', function(feedback, admin, oneS)
...
AddEventHandler('czReports:FeedbackConclude', function(feedbackID, info)
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "newFeedback" then
		exports['okokNotify']:Alert("czReports", "Report successfully sent to the STAFF!", 20000, 'success')
		
		local feedbackInfo = {subject = data.subject, information = data.information, category = data.category}
		TriggerServerEvent("czReports:NewFeedback", feedbackInfo)
	...
	elseif data.action == "assistFeedback" then
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				TriggerServerEvent("czReports:AssistFeedback", data.feedbackid, true)
			else
				...
				if playerOnline then
					...
					exports['okokNotify']:Alert("czReports", "That player is no longer in the server!", 20000, 'error')
				end
			end
		end
	elseif data.action == "concludeFeedback" then
		...
		if feedbackInfo then
			if feedbackInfo.concluded ~= true or canConclude then
				TriggerServerEvent("czReports:FeedbackConclude", feedbackID, canConclude)
